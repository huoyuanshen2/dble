...
public final class RouterUtil {
	...
	public static RouteResultset routeFromParserComplex(DruidParser druidParser, Map<String, SchemaConfig> schemaMap,
		...
		for (RouteCalculateUnit unit : druidParser.getCtx().getRouteCalculateUnits()) {
			RouteResultset rrsTmp = RouterUtil.tryRouteForTablesComplex(schemaMap, druidParser.getCtx(), unit, rrs,
					isSelect(statement), cachePool, node);
	...
	public static RouteResultset routeFromParser(DruidParser druidParser, SchemaConfig schema, RouteResultset rrs,
			SQLStatement statement, String originSql, LayerCachePool cachePool, ServerSchemaStatVisitor visitor,
	...
	public static void routeToSingleDDLNode(SchemaInfo schemaInfo, RouteResultset rrs, String dataNode)
			throws SQLException {
	...
	public static void routeNoNameTableToSingleNode(RouteResultset rrs, SchemaConfig schema)
	...
	public static void routeToRandomNode(RouteResultset rrs, SchemaConfig schema, String tableName)
			throws SQLException {
	...
	public static Set<String> ruleByJoinValueCalculate(String sql, TableConfig tc, Set<ColumnRoutePair> colRoutePairSet)
			throws SQLNonTransientException {
	...
	public static Set<String> ruleCalculate(TableConfig tc, Set<ColumnRoutePair> colRoutePairSet) {
		...
		for (ColumnRoutePair colPair : colRoutePairSet) {
			if (colPair.colValue != null) {
				...
				Integer[] nodeRange = algorithm.calculateRange(String.valueOf(colPair.rangeValue.getBeginValue()),
						String.valueOf(colPair.rangeValue.getEndValue()));
				if (nodeRange != null) {
					/**
	 ...
	 * 尝试路由多个表到一个节点。
	 */
	public static String tryRouteTablesToOneNode(String user, RouteResultset rrs, SchemaConfig schemaConfig,
			DruidShardingParseInfo ctx, Set<String> schemaList, int tableSize, boolean isSelect) throws SQLException {
			...
			if (tablesAndConditions != null) {
				for (Map.Entry<String, Map<String, Set<ColumnRoutePair>>> entry : tablesAndConditions.entrySet()) {
					...
					if (tableConfig == null) {
						...
						if (!tryCalcNodeForShardingTable(rrs, tmpResultNodes, tablesSet, entry, tableName, schema,
								tableConfig, isSelect)) {
							return null;
						}
					}
				}
	...
	private static boolean tryCalcNodeForShardingTable(RouteResultset rrs, Set<String> resultNodes,
			Set<String> tablesSet, Map.Entry<String, Map<String, Set<ColumnRoutePair>>> entry, String tableName,
			SchemaConfig schema, TableConfig tableConfig, boolean isSelect) throws SQLNonTransientException {
		Map<String, Set<ColumnRoutePair>> columnsMap = entry.getValue();

		Map<String, Set<String>> tablesRouteMap = new HashMap<>();
		if (tryRouteWithPrimaryCache(rrs, tablesRouteMap, CacheService.getTableId2DataNodeCache(), columnsMap, schema,
				tableName, tableConfig.getPrimaryKey(), isSelect)) {
	...
	public static RouteResultset tryRouteForTables(SchemaConfig schema, DruidShardingParseInfo ctx,
		...
		if (tablesAndConditions != null && tablesAndConditions.size() > 0) {
			...
			RouterUtil.findRouterWithConditionsForTables(schema, rrs, tablesAndConditions, tablesRouteMap, cachePool,
					isSelect, false, node);
			if (rrs.isFinishedRoute()) {
				return rrs;
			}
		...
		// if global table and normal table has no intersection ,they had treat as
		// normal join
		for (String tableName : tables) {
			...
			TableConfig tableConfig = schema.getTables().get(tableName);
			if (tableConfig != null && !tableConfig.isGlobalTable() && tablesRouteMap.get(tableName) == null) { // the
																												// other
																												// is
																												// single
																												// table
	 ...
	 * tryRouteFor multiTables
	 */
	public static RouteResultset tryRouteForTablesComplex(Map<String, SchemaConfig> schemaMap,
			DruidShardingParseInfo ctx, RouteCalculateUnit routeUnit, RouteResultset rrs, boolean isSelect,
		...
		if (tables.size() == 1) {
			return RouterUtil.tryRouteForOneTable((schemaMap.get(tables.get(0))), routeUnit, tables.get(0), rrs,
					isSelect, cachePool, node);
		}
		...
		if (tablesAndConditions != null && tablesAndConditions.size() > 0) {
			// findRouter for shard-ing table
			RouterUtil.findRouterWithConditionsForTables(schemaMap, rrs, tablesAndConditions, tablesRouteMap, cachePool,
					isSelect, false, node);
			if (rrs.isFinishedRoute()) {
				return rrs;
			}
	 ...
	 * tryRouteForOneTable
	 */
	public static RouteResultset tryRouteForOneTable(SchemaConfig schema, RouteCalculateUnit routeUnit,
			String tableName, RouteResultset rrs, boolean isSelect, LayerCachePool cachePool, PlanNode node)
			throws SQLException {
		...
		if (tc.isGlobalTable()) {
			...
			if (!checkRuleRequired(schema, routeUnit, tc)) {
				throw new IllegalArgumentException(
						"route rule for table " + tc.getName() + " is required: " + rrs.getStatement());
			...
			if ((tc.getPartitionColumn() == null && tc.getParentTC() == null)
				...
				if (routeUnit.getTablesAndConditions() != null && routeUnit.getTablesAndConditions().size() > 0) {
					RouterUtil.findRouterWithConditionsForTables(schema, rrs, routeUnit.getTablesAndConditions(),
							tablesRouteMap, cachePool, isSelect, true, node);
					if (rrs.isFinishedRoute()) {
						return rrs;
					}
				}
	...
	private static boolean tryRouteWithPrimaryCache(RouteResultset rrs, Map<String, Set<String>> tablesRouteMap,
			LayerCachePool cachePool, Map<String, Set<ColumnRoutePair>> columnsMap, SchemaConfig schema,
			String tableName, String primaryKey, boolean isSelect) {
	...
	public static void findRouterWithConditionsForTables(Map<String, SchemaConfig> schemaMap, RouteResultset rrs,
			...
			LayerCachePool cachePool, boolean isSelect, boolean isSingleTable, PlanNode node)
			throws SQLNonTransientException {
		...
		for (Map.Entry<String, Map<String, Set<ColumnRoutePair>>> entry : tablesAndConditions.entrySet()) {
			...
			if (tableConfig.isGlobalTable() || schema.getTables().get(tableName).getDataNodes().size() == 1) {
				...
				Map<String, Set<ColumnRoutePair>> columnsMap = entry.getValue();
				if (tryRouteWithPrimaryCache(rrs, tablesRouteMap, cachePool, columnsMap, schema, tableName,
						tableConfig.getPrimaryKey(), isSelect)) {
					continue;
				}
	...
	private static void routeWithPartition(Map<String, Set<String>> tablesRouteMap, String tableName,
			TableConfig tableConfig, Set<ColumnRoutePair> partitionValue, PlanNode pnode)
			throws SQLNonTransientException {
		for (ColumnRoutePair pair : partitionValue) {
			AbstractPartitionAlgorithm algorithm = tableConfig.getRule().getRuleAlgorithm();
			if (pnode != null && "null".equals(pair.colValue)) {
				...
				if (nodeIndex >= 0 && nodeIndex < dataNodes.size()) {
					...
					node = null;
					String msg = "Can't find a valid data node for specified node index :" + tableConfig.getName()
							+ " -> " + tableConfig.getPartitionColumn() + " -> " + pair.colValue + " -> " + "Index : "
							+ nodeIndex;
					LOGGER.info(msg);
					throw new SQLNonTransientException(msg);
				}
			...
			if (pair.rangeValue != null) {
				Integer[] nodeIndexes = algorithm.calculateRange(pair.rangeValue.getBeginValue().toString(),
						pair.rangeValue.getEndValue().toString());
				ArrayList<String> dataNodes = tableConfig.getDataNodes();
				String node;
	...
	private static void routerForJoinTable(RouteResultset rrs, TableConfig tableConfig,
			Map<String, Set<ColumnRoutePair>> columnsMap, String joinKey) throws SQLNonTransientException {
		// childTable (if it's ER JOIN of select)must find root table,remove childTable,
		// only left root table
		Set<ColumnRoutePair> joinKeyValue = columnsMap.get(joinKey);
		Set<String> dataNodeSet = ruleByJoinValueCalculate(rrs.getStatement(), tableConfig, joinKeyValue);
